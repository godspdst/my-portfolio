...
function FullThumb({ bg, logoPath, wmOpacity = 0.12, wmColor, logoShape = "bare" }) {
  // The Pit — bare lime EQ bars, no container, same size as other logos (~90px max)
  if (logoShape === "pit") {
    ...
    return (
      <div style={{height:"240px",position:"relative",background:C.ink,borderBottom:`1.5px solid ${C.ink}`,overflow:"hidden"}}>
        ...
        <div style={{position:"absolute",right:"-10px",top:"50%",transform:"translateY(-50%)",opacity:.12,display:"flex",alignItems:"flex-end",gap:"9px",pointerEvents:"none"}}>
          {bars.map((h,i) => <div key={i} style={{width:"26px",height:`${Math.round(h*5.4)}px`,background:C.lime,borderRadius:"5px 5px 0 0"}}/>)}
        </div>
        {/* Bright EQ bars — max height ~90px matching other logos */}
        <div className="pc-pit" style={{position:"absolute",left:"28px",top:"50%",transform:"translateY(-50%)",display:"flex",alignItems:"flex-end",gap:"7px"}}>
          {bars.map((h,i) => <div key={i} className="eq-bar" style={{width:"13px",height:`${Math.round(h*2.8)}px`,background:C.lime,borderRadius:"4px 4px 0 0",transformOrigin:"bottom"}}/>)}
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{height:"240px",position:"relative",background:bg,borderBottom:`1.5px solid ${C.ink}`,overflow:"hidden"}}>
      {/* Watermark */}
      <div style={{position:"absolute",right:"-30px",top:"50%",transform:"translateY(-50%)",width:"200px",height:"200px",opacity:wmOpacity,pointerEvents:"none"}}>
        <LogoSVG path={logoPath} color={wmColor||C.ink}/>
      </div>
      {/* Bare logo */}
      <div style={{position:"absolute",left:"28px",top:"50%",transform:"translateY(-50%)",width:"90px",height:"90px",zIndex:1}}>
...
const PROJECTS = [
  {
    ...
    locked:true, password:"designedx-byyy-st3phani3?", num:"01",
  },
  {
    ...
    locked:true, password:"made-byyy-st3phani3!", num:"02",
  },
  {
...
const PAGE_CSS = `
  ...
  .proj-card.jd-locked .lock-ovl{opacity:0;transition:opacity .2s ease;}
  .proj-card.jd-locked .lock-circ{transform:scale(.7);transition:transform .2s ease;}
  .proj-card.jd-locked:hover .lock-ovl{opacity:1;}
  .proj-card.jd-locked:hover .lock-circ{transform:scale(1);}
  @media(max-width:900px){.projects-grid{grid-template-columns:1fr!important}}
`;

function ProjectCard({ project: p, onOpenModal }) {
  return (
    <a href={`/case-study/${p.id}`}
      className={`proj-card${p.locked ? " jd-locked" : ""}`}
      style={{display:"block",textDecoration:"none",background:C.card,borderRadius:"12px",border:`1.5px solid ${C.ink}`,overflow:"hidden"}}
      onClick={p.locked ? e => { e.preventDefault(); onOpenModal(p); } : undefined}
    ...
    >
      ...
      <div style={{position:"relative"}}>
        <FullThumb bg={p.bg} logoPath={p.logoPath} wmOpacity={p.wmOpacity} wmColor={p.wmColor} logoShape={p.logoShape}/>
        {p.locked && <LockBadge fontSize={10.5}/>}
        {p.locked && <LockOverlay circleSize={56}/>}
      </div>
      {/* Body */}
...
export default function ProjectsPage() {
  ...
  return (
    <div style={{background:C.paper,minHeight:"100vh"}}>
      ...
      <div style={{maxWidth:"1700px",margin:"0 auto",padding:`24px ${PAGE_X} 44px`}}>
      ...
      <div className="projects-grid" style={{maxWidth:"1700px",margin:"0 auto",padding:`0 ${PAGE_X} 72px`,display:"grid",gridTemplateColumns:"1fr 1fr",gap:"24px"}}>
        {PROJECTS.map(p => <ProjectCard key={p.id} project={p} onOpenModal={setModal}/>)}
      </div>
